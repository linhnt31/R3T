...
contract Federation{
    struct Weight{ //Local model updates
        uint dataSize;
        string filePath;
        string fileHash;
    }
    ...
    // Add & Get Weights
    // *memory* keyword: store the data temporarily during the execution of a smart contract
    ...
    // Add & Get Strategy
    function addStrategy(uint _session, string memory _algoName, uint _numRounds, uint _numClients) public returns(string memory no){
        strategies[_session].algoName = _algoName;
        strategies[_session].numRounds = _numRounds;
        strategies[_session].numClients = _numClients;
        
        // Off-chain client applications can subscribe and listen for any events that are being emitted in the blockchain via RPC
        emit addStrategyEvent(_algoName, _numRounds, _numClients);
        
        return string.concat("Strategy", _algoName, "added successfully.");
    }
